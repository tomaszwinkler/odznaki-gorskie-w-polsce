...
PWA (React + Vite) for tracking progress toward Polish mountain-hiking badges (GOT, Korona Gór Polski, Diadem Polskich Gór, Korona Najwybitniejszych Szczytów Gór Polskich, four "korony makroregionalne" — Korona Sudetów, Korona Beskidów, Wielka Korona Beskidów, Turystyczna Korona Tatr — and four regional badges: Korona Bieszczadów, Dominanty Przedgórza Sudeckiego, Korona Polskich Beskidów, Mała Korona Beskidów). All data lives client-side in IndexedDB (Dexie) — no backend of our own; accounts and cross-device sync are optional and go through Dexie Cloud (see "Optional cloud sync"). UI text, data, comments, commit messages, and test descriptions are all in Polish; keep it that way.
